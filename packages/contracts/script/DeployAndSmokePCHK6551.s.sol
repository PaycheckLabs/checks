...
contract DeployAndSmokePCHK6551 is Script {
    ...
    function run() external {
        ...
        // 2) Deploy PCHK bound to ERC-6551 Registry
        PaymentChecksPCHK pchk = new PaymentChecksPCHK(
        ...
        musd.approve(address(pchk), type(uint256).max);

        bytes32 serial = bytes32("AMOY-DEMO-0001");
        bytes32 title  = bytes32("Demo Payment");

        (uint256 checkId, address account) = pchk.mintPaymentCheck(
            holder,
            AMOUNT,
            0,      // instant claim
            serial,
        ...
        console2.log("PCHK:", address(pchk));
